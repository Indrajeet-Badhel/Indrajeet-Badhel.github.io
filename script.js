...
function renderProjects() {
  ...
  target.innerHTML = data.projects.map((project) => {
    const stack = project.stack.slice(0, 5).map((item) => `<span>${item}</span>`).join("");
    ...
    return `
      <article class="project-card">
        <a class="project-shot" href="${project.link}" ${project.link === "#" ? "" : 'target="_blank" rel="noreferrer"'} aria-label="Open ${project.name}">
